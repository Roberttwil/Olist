...
def planner_node(state: AgentState) -> Dict[str, Any]:
    ...
    system_prompt = (
        ...
        "3. Always specify to LIMIT the results in the description if the query asks for top items or rankings (e.g. limit to 1 or 5 rows) to avoid loading too many rows.\n"
        "4. CLARIFICATION & OUT-OF-SCOPE HANDLING: \n"
        "   - If the user query is completely out-of-scope (e.g. general knowledge questions like 'unpad apaan', 'siapa presiden RI', etc.), do NOT generate SQL tasks. Generate a single task object with description explaining politely that the question is not relevant to the Olist e-commerce dataset and you cannot answer it. Example: {\"task_id\": 1, \"description\": \"Maaf, pertanyaan Anda mengenai Universitas Padjadjaran (UNPAD) tidak relevan dengan analisis data transaksi e-commerce Olist. Saya hanya dapat menjawab pertanyaan seputar data penjualan, produk, pelanggan, ulasan, dan kinerja operasional Olist.\", \"status\": \"clarification_needed\", \"requires_approval\": false, \"sql_query\": \"\", \"sql_result\": null, \"error_message\": \"\", \"retry_count\": 0}\n"
        "   - If the user query is about the database but is extremely ambiguous, generate a single task object with description set to a polite clarifying question. Example: {\"task_id\": 1, \"description\": \"Maaf, apakah data penjualan yang Anda maksud adalah total omset (payment_value) atau jumlah item terjual?\", \"status\": \"clarification_needed\", \"requires_approval\": false, \"sql_query\": \"\", \"sql_result\": null, \"error_message\": \"\", \"retry_count\": 0}\n"
        "5. HISTORICAL DATASET & RELATIVE TIME: The Olist database contains static historical data from 2016 to 2018. If the user asks questions containing relative time expressions like 'bulan lalu', 'minggu ini', 'tahun lalu', or 'kemarin' without specifying a year, you MUST NOT generate SQL tasks. Instead, generate a single task object with description set to a polite clarifying question asking which specific historical month/year they want to check (reminding them that the last complete month is August 2018), and status set to \"clarification_needed\". Example: {\"task_id\": 1, \"description\": \"Dataset Olist berisi data historis dari tahun 2016 hingga 2018. Periode transaksi terakhir yang tersedia adalah Agustus 2018. Apakah yang Anda maksud dengan 'bulan lalu' adalah Agustus 2018, or bulan dan tahun spesifik lainnya?\", \"status\": \"clarification_needed\", \"requires_approval\": false, \"sql_query\": \"\", \"sql_result\": null, \"error_message\": \"\", \"retry_count\": 0}\n"
        "6. GREETINGS & SMALL TALK: If the user query is a simple greeting (like 'hi', 'hello', 'halo', 'siang') or small talk, do NOT generate SQL tasks. Instead, generate a single task object with description set to a warm, friendly greeting in Indonesian and a brief explanation of what you can help with, and status set to \"clarification_needed\". Example: {\"task_id\": 1, \"description\": \"Halo! Saya adalah asisten analisis data e-commerce Olist. Saya bisa membantu Anda mencari data penjualan, performa produk, ulasan pelanggan, dan lainnya. Ada yang ingin Anda tanyakan?\", \"status\": \"clarification_needed\", \"requires_approval\": false, \"sql_query\": \"\", \"sql_result\": null, \"error_message\": \"\", \"retry_count\": 0}\n"
...
def task_executor_node(state: AgentState) -> Dict[str, Any]:
    ...
    system_prompt = (
        ...
        "Example 1: Get average review score per product category\n"
        "Correct Path: Join v_order_reviews_detailed and v_order_items_detailed on order_id\n"
        "SELECT oi.product_category_name_english, AVG(r.review_score) as avg_score\n"
        ...
        "Example 2: Find total sales revenue per seller in a specific state\n"
        ...
        "GROUP BY seller_id;\n\n"
        "Example 3: Find the number of unique customers who placed more than one order (repeat buyers)\n"
...
def global_synthesizer_node(state: AgentState) -> Dict[str, Any]:
    ...
    if any(
        ...
        return {"final_answer": "Eksekusi SQL dibatalkan oleh pengguna."}
    ...
    if plan and plan[0]["status"] == "clarification_needed":
        ...
        clarifying_question = plan[0].get("description") or plan[0].get("sql_result") or "Maaf, bisakah Anda memperjelas pertanyaan Anda?"
